...
export function IndiaIncomeTaxForm({ taxData }: Props) {
  ...
  const fmt = (amount: number) => formatCurrency(amount, "INR", "en-IN");

  const breakdownItems: BreakdownItem[] = result
    ? [
        { label: t("incomeTax"), amount: result.incomeTax, color: "#ef4444" },
        ...(result.surcharge > 0
          ? [{ label: t("surcharge"), amount: result.surcharge, color: "#f97316" }]
          : []),
        { label: t("cess"), amount: result.cess, color: "#a855f7" },
        { label: t("epf"), amount: result.epf, color: "#3b82f6" },
        { label: t("professionalTax"), amount: result.professionalTax, color: "#06b6d4" },
      ]
    : [];
  ...
  const tableItems: BreakdownItem[] = result
    ? [
        { label: t("incomeTax"), amount: result.incomeTax, color: "#ef4444" },
        ...(result.rebate87A > 0
          ? [{ label: t("rebate87A"), amount: -result.rebate87A, color: "#22c55e" }]
          : []),
        ...(result.surcharge > 0
          ? [{ label: t("surcharge"), amount: result.surcharge, color: "#f97316" }]
          : []),
